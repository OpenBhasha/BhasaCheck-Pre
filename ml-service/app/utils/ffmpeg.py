...
FFmpeg is used only for mechanical audio manipulation (resample, cut,
format conversion) — it never determines timestamps itself. The ML models
(Silero VAD, pyannote, Whisper/IndicConformer/NeMo) decide *where* to cut;
this module just performs the cut.
"""
...
def to_wav_mono_16k(input_path: str, output_path: str) -> None:
    """Normalizes to the format most local speech models expect: mono, 16kHz, PCM WAV."""
    subprocess.run(
        [
